...
bp_xmax=df4.groupby(['Area']).GlobalX.max()
bp_ymin=df4.groupby(['Area']).GlobalY.min()
bp_ymax=df4.groupby(['Area']).GlobalY.max()
...
ppc = pd.DataFrame()
ppc_name = []
ppc=df3[df3.Point.str.contains('P')].reset_index(drop=True)
